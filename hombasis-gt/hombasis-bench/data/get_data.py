...
def add_zinc_subhom(
    name, hom_files, idx_list, sub_file, root, dataset, spasm_count_label=False
):
    if name == "ZINC":
...
def add_zinc_hom(
    ...
    root,
    dataset,
    wl_count_label=False,
    preserve_counts_spasm=False,
):
    if name == "ZINC":
        original_data = [dataset[i] for i in range(len(dataset))]
...
def add_lpca(root, dataset):
    ...
    for i in range(len(dataset)):
        m = matrices[f"idx_{i}"]
        data = dataset[i]
        result.append(Data(**data, lpca_enc=torch.Tensor(m)))
