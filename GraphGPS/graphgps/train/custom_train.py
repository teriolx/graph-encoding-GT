...
def get_QM9_denorm_cons():
    assert "QM9" in cfg.dataset.format 
    ...
    assert cfg.dataset.name in targets
    idx = targets.index(cfg.dataset.name)
    return norms[idx]

class ParamBudget:#instantiate this object in custom train prior to any epochs, run check_budget after first val epoch
    def __init__(self,budget,checked=False, wb_log=None):
        self.checked = checked
        self.budget = budget
        self.wb_log = wb_log
    def check_budget(self, model):
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        if total_params > self.budget:
            if self.wb_log != None:
                self.wb_log.summary["over_param_budget"] = True
                self.wb_log.finish()
            raise Exception('Over Param Budget')
        self.checked = True
...
@torch.no_grad()
def eval_epoch(logger, loader, model, split='val',param_budget=None): #EDITTED to add check for param budget
    model.eval()
    time_start = time.time()
    for batch in loader:
        batch.split = split
        ...
        if cfg.dataset.name == 'ogbg-code2':
            ...
            loss, pred_score = compute_loss(pred, true)
            if "QM9" in cfg.dataset.format: #EDITTED added to de-nomralize y targets on evaluation
                extra_stats["real_MAE"] = loss.detach().cpu().item() / get_QM9_denorm_cons()
            _true = true.detach().to('cpu', non_blocking=True)
            _pred = pred_score.detach().to('cpu', non_blocking=True)
        logger.update_stats(true=_true,
                            pred=_pred,
    ...
    if param_budget != None:
        if not param_budget.checked:
            param_budget.check_budget(model=model)
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    if hasattr(cfg.train, "enforce_param_budget") and cfg.train.enforce_param_budget: #EDITED added for param_budget
        assert hasattr(cfg.train, "param_budget"), "must provide a cfg.train.param_budget to enforce"
        param_budget = ParamBudget(budget=cfg.train.param_budget, wb_log=run) if cfg.wandb.use else ParamBudget(budget=cfg.train.param_budget)
    else:
        param_budget = None
        ...
        if is_eval_epoch(cur_epoch):
            for i in range(1, num_splits):
                eval_epoch(loggers[i], loaders[i], model,
                           split=split_names[i - 1], param_budget=param_budget)
        ...
        if is_eval_epoch(cur_epoch):
                ...
                if cfg.wandb.use:
                    ...
                    for i, s in enumerate(['train', 'val', 'test']):
                        ...
                        if m in perf[i][best_epoch]:
                            ...
                            if "QM9" in cfg.dataset.format: #EDITTED added to normalize mae
                                bstats[f"best/real_{s}_{m}"] = perf[i][best_epoch][m] / get_QM9_denorm_cons()
            ...
            if hasattr(model.model, 'trf_layers'):
                ...
                for li, gtl in enumerate(model.model.trf_layers):
                    if hasattr(gtl.attention, 'gamma') and \
                        torch.is_tensor(gtl.attention.gamma) and \
