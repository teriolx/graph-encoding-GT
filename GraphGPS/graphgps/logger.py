...
class CustomLogger(Logger):
    ...
    # def regression(self): #EDITTED to add if block checking is true or pred is NaN (NaN input was causing issues I think)
    #     true, pred = torch.cat(self._true), torch.cat(self._pred)
    #     reformat = lambda x: round(float(x), cfg.round)
    #     if math.isnan(true) or math.isnan(pred):
    #         print(f'TRUE OR PRED IS NAN') #likely caused by some numerical issue in the foward computation of pred
    #         print(f'true: {true}')
    #         print(f'pred: {pred}')
    #         return {
    #         'mae': float('nan'),
    ...
    def write_epoch(self, cur_epoch):
        ...
        # json
        #dict_to_json(stats, '{}/stats.json'.format(self.out_dir)) #EDITTED OUT because causing bugs with large paralell jobs
        # tensorboard
