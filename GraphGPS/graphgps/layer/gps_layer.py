...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        self.n_edge_types = n_edge_types #for RGCNConv
        ...
        # MPNNs supporting also edge attributes.
        elif local_gnn_type == 'RGCNConv': #added for qm9
            assert self.n_edge_types != None, "Must provide cfg.gnn.edge_encoder_num_types to use RGCN. Must also make sure cfg.dataset_edge_encoder is False (do not need edge embeddings to use RGCN, just need types)."
            self.local_model = pygnn.RGCNConv(dim_h, dim_h, self.n_edge_types, aggr='add')
            ...
            if self.equivstable_pe:  # Use specialised GINE layer for EquivStableLapPE.
                ...
                self.local_model = pygnn.GINEConv(gin_nn) if self.initial_edge_dim == None else pygnn.GINEConv(gin_nn, edge_dim=self.initial_edge_dim)
        ...
        # Feed Forward block.
        assert not (self.ff == False and self.down_proj != None), "Must enable ff in order to use down projection"
        if self.ff:
            self.ff_linear1 = nn.Linear(dim_h, dim_h * 2)
            self.ff_linear2 = nn.Linear(dim_h * 2, dim_h)
            self.act_fn_ff = self.activation()
            if self.layer_norm:
                dim_ln = dim_h if self.down_proj == None else self.down_proj['dim_hidden']
                self.norm2 = pygnn.norm.LayerNorm(dim_ln)
                # self.norm2 = pygnn.norm.GraphNorm(dim_h)
                # self.norm2 = pygnn.norm.InstanceNorm(dim_h)
            if self.batch_norm:
                dim_bn = dim_h if self.down_proj == None else self.down_proj['dim_hidden']
                self.norm2 = nn.BatchNorm1d(dim_bn)
